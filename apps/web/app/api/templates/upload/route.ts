...
import { uploadTemplatePdf } from '@/src/server/storage/supabase';
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { url, path } = await uploadTemplatePdf(buffer, name);
    ...
    const template = await prisma.documentTemplate.create({
      data: {
        ...
        pdfUrl: url,
        cloudinaryPublicId: path, // Stocker le path Supabase pour compatibilité
